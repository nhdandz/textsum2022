        ...
        if ischeck == True:
            ...
            try:
                ...
                if file_type == PDF:
                    ...
                    text = process_doc_3_text(temp.name,page_from,page_to)
                else:
                    text = process_doc_text(temp.name,page_from,page_to)
                return text
            finally:
                # Đảm bảo đóng và xóa file tạm
                temp.close()
                try:
                    os.unlink(temp.name)
                except:
        ...
        if ischeck == True:
            ...
            try:
                ...
                if file_type == PDF:
                    ...
                    if page > 2:
                        text_page_2 = process_doc_text(temp.name, page-3, page-3)
                        text_page_1 = process_doc_text(temp.name, page-2, page-2)
...
def process_pdf_text(file_name,page_from,page_to):
    ...
    if len(all_text.split(' ')) / len(all_text_unprocess.split(' ')) <= 0.35:
        ...
        text = read_pdf_by_convert_docx(file_name,page_from,page_to)
        return text
...
def process_doc_text(file_name,page_from,page_to):
    ...
    if (page_to == pageCount-1 and page_from == 0)or page_to > pageCount:
        ...
        if page_to == page_from:
            ...
            extractedPages.save(temp_doc.name+'.docx')
        else:
            extractedPages = doc.extract_pages(page_from, (page_to-page_from+1))
            extractedPages.save(temp_doc.name+'.docx')
        file_name = temp_doc.name+'.docx'
    docx_read = docx.Document(file_name)
    all_paras = docx_read.paragraphs
    all_text = ''
    all_text_list_process = []
    stop_paragraph = ('...', '.', '?', '!', '!!!', ':','."')
    for para in all_paras:
        ...
        if len(text.split(' ')) > 15 :
            all_text_list_process.append(text)
        elif len(text.split(' ')) > 10 :
            if text[0].isupper() == True and text[-1] in stop_paragraph:
                continue
            elif text[0].isupper() == True or text[-1] in stop_paragraph or (text[-1].isnumeric() == True and text[-2] in stop_paragraph):
                all_text_list_process.append(text)
            elif text[0].isupper() == True or text[-1] in stop_paragraph or (text[-1].isnumeric() == True and text[-2].isnumeric()== True and text[-3] in stop_paragraph):
                all_text_list_process.append(text)
            elif text[0].isupper() == True or text[-1] in stop_paragraph or (text[-1].isnumeric() == True and text[-2].isnumeric()== True and text[-3].isnumeric()== True and text[-4] in stop_paragraph):
                all_text_list_process.append(text)
        else:
            continue
...
def process_doc_3_text(file_name,page_from,page_to):
    doc = aw.Document(file_name)
    pageCount = doc.page_count
    ...
    if (page_to == pageCount-1 and page_from == 0) or page_to >= pageCount:
        # print('the same ext doc')
        extractedPages = doc.extract_pages(page_from,pageCount-1)
    ...
    for idx, paragraph in enumerate(all_text_list_process):
        try:
            if paragraph[0].isupper() == True and paragraph[-1] in stop_paragraph:
                ...
                continue
    return all_text
def read_pdf_by_convert_docx(file_name,page_from,page_to):
    pdf = Pdf.open(file_name)
    if page_to > len(pdf.pages)-1:
        page_to = len(pdf.pages)-1
    page_number = page_to - page_from 
    file2pages = {}
    page_split = 10
    text_all = ""
    temp_number =  page_number//page_split
    
    if temp_number == 0:
        file2pages[0] = [page_from,page_to+1]
    else:
        temp = page_from

        for i in range(temp_number+1):
            if i == temp_number:
                file2pages[i] = [temp+page_split*i,page_to+1]
            else:
                file2pages[i] = [temp+page_split*i,temp+page_split*(i+1)]
    print(file2pages)
    new_pdf_files = [ Pdf.new() for i in file2pages ]
    new_pdf_index = 0
    for i in range(len(file2pages)):
        for n, page in enumerate(pdf.pages):
            if n in list(range(*file2pages[i])):
                new_pdf_files[i].pages.append(page)
        temp_pdf = tempfile.NamedTemporaryFile(suffix='.pdf', delete=True)
        new_pdf_files[i].save(temp_pdf.name)
        temp_doc = tempfile.NamedTemporaryFile(suffix='.docx', delete=True)
        doc = aw.Document(temp_pdf.name)
        temp_pdf.close()
        # Save the pdf to DOCX format.
        doc.save(temp_doc.name)
        text = process_doc_text(temp_doc.name,0,9999)
...
# file = '/home/nhdandz/Documents/tupk/textsum2022/modules/single_kafka/CV_PHAM_KHAC_TU.pdf'
# text =  process_pdf_text(file,7,18)
# text_file = open("result_231.txt", "w")

# #write string to filea

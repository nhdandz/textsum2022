...
def main():
    ...
    for data in consumer:
        ...
        try:
            res1 = requests.get(f"{url_status}/{sum_id}")
            res1 =json.loads(res1.content)
            # thuc hien ko tom tat
            if not res1["data"]["status"]:
                consumer.commit()
                continue
        except:
            consumer.commit()
        ...
        try:
            ...
            if message["is_cluster"]:
                ...
                try:
                    ...
                    consumer.commit()
                    logging.info("send output")
                    # logging.info(output_format)
                ...
                try:
                    for top in message["topic"]:
                        ...
                        tic = time.time()
                        summary = LSASummary(top["list_doc"], per)
                        toc = time.time()
                        print(f"time infer {toc -tic}")
                        if summary is None:
                            summary = ""

    ...
    if request.method =="POST":
        ...
        if content['list_doc'] is not None:
            ...
            if content['percent_output'] is not None:
                ...
                per = 0.1
            summary = TexRankSummary(content['list_doc'], per)
            # texts2 = prep(content['list_doc'])
            # texts2 = texts2.replace("||||", ". ||||")
            # texts = ""
            # for x in content['list_doc']:
            #     texts= texts +"||||"+ x
            # # print(texts)
            # counter = count_sentences(texts2, per)
            # print(counter)
            # extracted_sen =luhn_text_sum2(texts2, counter )
            # print(extracted_sen)
            # summary = sortByDocToStr(texts, extracted_sen)
            # # summary =  luhn_text_sum(texts, short_count_sentences(texts, per))
    ...
    if request.method =="POST":
        ...
        if content['list_doc'] is not None:
            if content['percent_output'] is not None:
                ...
                per = 0.1
            summary = LexRankSummary(content['list_doc'], per)
            # texts = prep(content['list_doc'])
            # print(texts)
            # extracted_sen =lexrank_extract_sum2(texts, short_count_sentences(texts, per))
    ...
    if request.method =="POST":
        ...
        if content['list_doc'] is not None:
            if content['percent_output'] is not None:
                ...
                per = 0.1
            summary = LSASummary(content['list_doc'], per)
            result['result'] = summary
            print(result)
            return  result
        return result

...
def count_sentences(document, per):
    ...
    for t in texs4:
        for t1 in t:
            if (t1 != ''):
                texs5.append(t1)
    x = len(texs5)
    percent = round(x * percent_output)
    if percent ==0:
        percent +=1
    return percent

def normal_count_sentences(document, percent_output):
    texs1 = document.split("\n")
    texs2 = [t.split("\n") for t in texs1]
    texs3 = []
    le = 0
    ...
    for t in texs4:
        for t1 in t:
            if (t1 != ''):
                texs5.append(t1)
    x = len(texs5)
    return round(x * percent_output)+1

def smooth_sentence(sent):
    split = str(sent).strip().split(" ")
    txt_smooth = ""
    length = len(split)
    try:
        for i in range(1,length-1, 1):
            txt_smooth+=split[i] +" "
        return txt_smooth.strip()
...
def sortByDocToArr(para, sens):
    ...
    for sen in sens:
        sen_smooth = smooth_sentence(sen)
        for idx, para in enumerate(paras):
            if (sen_smooth in para):
                result[idx].append(sen.strip())
                break
    return result
